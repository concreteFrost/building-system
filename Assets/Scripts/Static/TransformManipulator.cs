...
public static class TransformManipulator
{
    ...
    public static void ResetPosition(Part part)
    {
        part.part.transform.localPosition = Vector3.zero;

        if(part.GetComponent<BuildingPart>() != null)
        {
            var buildingPart = part.GetComponent<BuildingPart>(); ;
            buildingPart.snapPoints.transform.localPosition = Vector3.zero;
        }
